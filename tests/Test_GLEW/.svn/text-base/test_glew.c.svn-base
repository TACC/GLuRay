...
if (vi == NULL) { vi = glXChooseVisual(dpy, DefaultScreen(dpy), attributeListDbl);
swap_flag = 1; } /* create a GLX context */ cx = glXCreateContext(dpy, vi, 0, GL_TRUE);
...
glCallList(list);
glFinish(); if (swap_flag) glXSwapBuffers(dpy,win);
}
