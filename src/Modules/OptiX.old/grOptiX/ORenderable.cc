...
void OGeometryGeneratorQuads::addNormal(Manta::Real x, Manta::Real y, Manta::Real z)
{
...
void OGeometryGeneratorQuads::addVertex(Manta::Real x, Manta::Real y, Manta::Real z)
{
  ...
  if (_vertCounter > 3 && _vertCounter%4 == 0) {
    ...
    // }
          if (_mesh->vertexNormals.size())
      {
        while (_mesh->vertexNormals.size() < _mesh->vertices.size())
        {
          _mesh->vertexNormals.push_back(_mesh->vertexNormals.back());
        }
      }
    //_mesh->addTriangle(prim);
...
void OGeometryGeneratorQuadStrip::addNormal(Manta::Real x, Manta::Real y, Manta::Real z)
{
