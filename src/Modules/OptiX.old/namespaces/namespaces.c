...
/*
 * namespaces.c -- (taken from sample1).  Intended to demonstrate explicit use of 
 * namespaces.
 *
...
static void printUsageAndExit( const char* argv0 )
{
  ...
  fprintf( stderr, "\n" );
  fprintf( stderr, "Standard keystrokes:\n" );
...
int main(int argc, char* argv[])
{
    ...
    RT_CHECK_ERROR( rtContextDeclareVariable( context, "buffers::result_buffer", &result_buffer ) );
    RT_CHECK_ERROR( rtVariableSetObject( result_buffer, buffer ) );

    sprintf( path_to_ptx, "%s/%s", sutilSamplesPtxDir(), "namespaces_generated_namespaces.cu.ptx" );
    RT_CHECK_ERROR( rtProgramCreateFromPTXFile( context, path_to_ptx, "function::draw_solid_color", &ray_gen_program ) );
    RT_CHECK_ERROR( rtProgramDeclareVariable( ray_gen_program, "color::draw_color", &draw_color ) );
    ...
    RT_CHECK_ERROR( rtProgramQueryVariable( ray_gen_program, "color::draw_color", &draw_color  ) );
    if (draw_color == 0) {
      sutilReportError("RTcontext variable color::draw_color was not found");
      return 2;
    }
    {
      float r,g,b;
      RT_CHECK_ERROR( rtVariableGet3f( draw_color, &r, &g, &b ) );
      if (r != 0.462f || g != 0.725f || b != 0.f ) {
        fprintf(stderr, "draw_color = (%f, %f, %f)\n", r, g, b);
        fprintf(stderr, "expected   : (%f, %f, %f)\n", 0.462f, 0.725f, 0.f);
        sutilReportError("draw_color doesn't match.  See stderr for details");
        return 2;
      }
    }
