...
//
// Phong shading with reflections and shadows 
// 
...
RT_PROGRAM void closest_hit_radiance()
{
  ...
  for(int i = 0; i < lights.size(); ++i) {
    ...
    if( nDl > 0.0f ) {
      ...
      optix::Ray shadow_ray( hit_point, L, 1 /* any_hit ray type */, scene_epsilon, Ldist );
  ...
  if( prd_radiance.depth < max_depth) {
    ...
    optix::Ray refl_ray( hit_point, R, 0 /* closest_hit ray type */, scene_epsilon, RT_DEFAULT_MAX );
